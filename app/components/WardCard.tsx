...
export default function WardCard({ ward, href }: WardCardProps) {
  ...
  return (
    <Link
      href={wardHref}
      className="block bg-primary-light/20 rounded-lg shadow-md px-6 py-4 cursor-pointer hover:shadow-lg transition-shadow border-l-4 border-blue-500"
    >
      <h2 className="text-xl font-bold text-gray-800 mb-2">{ward.name}</h2>

      <div className="flex w-full gap-4 mb-4">
        <div className="bg-green-50 flex items-center justify-center gap-3 w-full p-3 rounded">
          <p className="text-md text-gray-600">Available</p>
          <p className="text-md font-bold text-green-600">
            {ward.availableBeds}
          </p>
        </div>
        <div className="bg-blue-50 items-center justify-center gap-3 flex w-full p-3 rounded">
          <p className="text-md text-gray-600">Occupied</p>
          <p className="text-md font-bold text-blue-600">{ward.occupiedBeds}</p>
        </div>
        <div className="bg-yellow-50 items-center gap-3 justify-center flex w-full p-3 rounded">
          <p className="text-md text-gray-600">Maintenance</p>
        ...
        <div className="bg-purple-50 items-center gap-3 justify-center flex w-full p-3 rounded">
          <p className="text-md text-gray-600">In Queue</p>
      ...
      <p className="text-sm text-gray-600 ">Occupancy: {occupancyRate}%</p>
    </Link>
  );
}
