...
This module is the SINGLE SOURCE OF TRUTH for explanation logic. It is
imported both:
  1. from Google Colab (for offline SHAP analysis + report figures), and
  2. from mlops/scripts/explain.py (the CLI the Next.js web app calls).
...
def shap_explain_agents(actors, state, background_states=None, n_samples=100):
    ...
    if background_states is None:
        # Fallback: perturb the current state slightly to build a local
        # background set. Prefer passing real historical states when you
        # have them (see build_background_from_history in the Colab script).
...
if __name__ == "__main__":
    # Quick smoke test with a synthetic ward snapshot -- see explain.py for
    # the real CLI used by the web app.
    demo_snapshot = {
        "totalBeds": 37,
        "occupiedBeds": 30,
        "queue": [
            {"patientId": "p1", "name": "Lisa Rodriguez", "triageLevel": 3, "waitMinutes": 319},
            {"patientId": "p2", "name": "Joseph Brown", "triageLevel": 3, "waitMinutes": 65},
            {"patientId": "p3", "name": "David Lopez", "triageLevel": 1, "waitMinutes": 418},
        ],
    }
    print(json.dumps(explain_decision(demo_snapshot, "best_mappo_hospital.pth"), indent=2))