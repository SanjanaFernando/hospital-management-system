...
class ArrivalForecaster:
    ...
    def from_patient_records(
        ...
        crit_share_by_hour = {
            h: (crit_counts[h] / crit_totals[h] if crit_totals[h] else 0.2) for h in range(24)
        }
        return cls(
    ...
    @classmethod
    def from_profile_file(cls, profile_path: str | Path, horizon_hours: int = 6) -> "ArrivalForecaster":
...
def load_forecaster(
    ...
    """Load forecaster from ward payload history, explicit profile, or default JSON."""
    ...
    resolved_profile = (
        profile_path
        or ward_snapshot.get("forecasterProfilePath")
        or default_profile_path()
    )
    if Path(resolved_profile).exists():
        forecaster = ArrivalForecaster.from_profile_file(resolved_profile, horizon_hours=horizon)
        return forecaster
